...
/**
 * Fi CLI detector
 * Tries lobehub, lobe, lh in order; validates via --help output containing "Fi"
 */
  ...
  async detect(): Promise<ToolStatus> {
    ...
    for (const cmd of commands) {
      try {
        ...
        // Validate it's actually Fi CLI by checking help output
        const { stdout: helpOut } = await execPromise(`${cmd} --help`, { timeout: 3000 });
        if (!helpOut.includes('Fi')) continue;
