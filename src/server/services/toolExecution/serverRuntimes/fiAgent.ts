import type { VisualFileItem, VisualSourceMessage } from '@ficlouds/builtin-tool-fi-agent';
import {
...
} from '@ficlouds/builtin-tool-fi-agent';
...
import { createServerPlanRuntimeService } from './fiAgentPlan';
...
interface FiAgentRuntimeContext {
  agentId?: string | null;
  groupId?: string | null;
  messageId: string;
  serverDB: FiDatabase;
...
class FiAgentExecutionRuntime {
  ...
  constructor(context: FiAgentRuntimeContext) {
...
export const fiAgentRuntime: ServerRuntimeRegistration = {
  factory: (context) => {
    if (!context.serverDB) {
      throw new Error('serverDB is required for FiAgent execution');
    }
    if (!context.userId) {
      throw new Error('userId is required for FiAgent execution');
    }
    if (!context.messageId) {
      throw new Error('messageId is required for FiAgent execution');
    }

    return new FiAgentExecutionRuntime({
