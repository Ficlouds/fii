import { ARTIFACT_THINKING_TAG_REGEX } from '@ficlouds/const';

const ARTIFACT_TAG_REGEX_GLOBAL =
  /<lobeArtifact\b[^>]*>(?<content>[\S\s]*?)(?:<\/lobeArtifact>|$)/g;

// Match only the `fiAgents` tag itself (self-closing `/>` or a bare opening
...
// nothing to capture inside. A previous `>([\S\s]*?)(?:<\/fiAgents>|$)` form
// fell back to `$` when a model omitted the self-closing slash and emitted
// `<fiAgents ...>`; with no `</fiAgents>` to anchor on, it swallowed the
...
const AGENTS_TAG_REGEX_GLOBAL = /<fiAgents\b[^>]*>/g;
...
export const processWithArtifact = (input: string = '') => {
  ...
  // Strip newlines inside the fiAgents tag so attributes spread across lines
