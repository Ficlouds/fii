import { FiAgentIdentifier, MAX_VISUAL_MEDIA_URLS } from '@ficlouds/builtin-tool-fi-agent';
import { createVisualFileRef, RequestTrigger } from '@ficlouds/types';
...
const { fiAgentRuntime } = await import('../fiAgent');

describe('fiAgentRuntime', () => {
  ...
  it('should have the correct identifier', () => {
    expect(fiAgentRuntime.identifier).toBe(FiAgentIdentifier);
  });

  it('should require serverDB, userId and messageId', () => {
    expect(() => fiAgentRuntime.factory({ toolManifestMap: {}, userId: 'user-1' })).toThrow(
      'serverDB is required for FiAgent execution',
    );

    expect(() => fiAgentRuntime.factory({ serverDB: {} as any, toolManifestMap: {} })).toThrow(
      'userId is required for FiAgent execution',
    );

    expect(() =>
      fiAgentRuntime.factory({
        serverDB: {} as any,
        toolManifestMap: {},
        userId: 'user-1',
      }),
    ).toThrow('messageId is required for FiAgent execution');
  });
  ...
  it('should return a configuration error when visual model env is missing', async () => {
    ...
    const runtime = fiAgentRuntime.factory(baseContext);
  ...
  it('should resolve stable refs from earlier topic messages', async () => {
    ...
    const runtime = fiAgentRuntime.factory({ ...baseContext, topicId: 'topic-1' });
