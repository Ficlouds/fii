...
describe('heterogeneousAgentExecutor DB persistence', () => {
  ...
  describe('final content writes (onComplete)', () => {
    ...
    it('should ignore stale usage on assistant events (from message_start echo)', async () => {
      // Regression for CC partial-messages mode: under partial-messages mode, CC
  ...
  // ────────────────────────────────────────────────────
  // Real trace regression: multi-tool per turn (i18n translation CC scenario)
  // ────────────────────────────────────────────────────

  describe('multi-tool per turn (real trace regression)', () => {
    /**
     * Reproduces the exact CC event pattern from the i18n translation CC scenario.
    ...
    it('should register ALL tools on correct assistant when turn has text + multiple tool_uses', async () => {
      ...
      await runWithEvents([
        ...
        ccToolUse('msg_03', 'toolu_getissue', 'mcp__linear__get_issue', { id: 'CC-i18n' }),
  ...
  // ────────────────────────────────────────────────────
  // CC desktop integration reproduction: Skill → ToolSearch → MCP tool
  //
  ...
  describe('CC desktop Skill → ToolSearch → MCP repro', () => {
  ...
  // ────────────────────────────────────────────────────
  // CC subagent thread-container model (inline rendering + subagent lineage)
  //
  ...
  describe('CC subagent thread-container', () => {
    ...
    /**
     * Regression for subagent event leak: the forwarding guard initially only
  ...
  describe('Monitor parentId chain', () => {
    ...
    /**
     * Toolless middle step regression: a toolless step in the middle must NOT break the
    ...
    /**
     * Toolless middle step follow-up: N consecutive toolless steps (Monitor pushing
  ...
  // ────────────────────────────────────────────────────
  // External signal stamping on Monitor-driven follow-up steps (Phase 1: metadata.signal)
  // ────────────────────────────────────────────────────

  describe('external signal (metadata.signal) for Monitor callbacks', () => {
    ...
    it('stamps metadata.signal on assistant turns CC opens without user input while a task is active', async () => {
      ...
      // Step 4 post-task summary: tagged with `task-completion` signal
