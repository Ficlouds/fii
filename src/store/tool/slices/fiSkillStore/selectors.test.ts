...
describe('fiSkillStoreSelectors', () => {
  describe('getServers', () => {
    ...
    it('should return all servers', () => {
      ...
      const state = { ...initialState, fiSkillServers: servers } as ToolStore;
      const result = fiSkillStoreSelectors.getServers(state);
      expect(result).toEqual(servers);
    });

    it('should handle undefined fiSkillServers', () => {
      const state = { ...initialState, fiSkillServers: undefined } as unknown as ToolStore;
      const result = fiSkillStoreSelectors.getServers(state);
      expect(result).toEqual([]);
    });
  });
  ...
  describe('getConnectedServers', () => {
    it('should return only connected servers', () => {
      ...
      const state = { ...initialState, fiSkillServers: servers } as ToolStore;
      const result = fiSkillStoreSelectors.getConnectedServers(state);
  ...
  describe('getAllServerIdentifiers', () => {
    it('should return set of all server identifiers', () => {
      ...
      const state = { ...initialState, fiSkillServers: servers } as ToolStore;
      const result = fiSkillStoreSelectors.getAllServerIdentifiers(state);
  ...
  describe('getServerByIdentifier', () => {
    it('should return server by identifier', () => {
      ...
      const state = { ...initialState, fiSkillServers: servers } as ToolStore;
      const result = fiSkillStoreSelectors.getServerByIdentifier('linear')(state);
    ...
    it('should return undefined when server not found', () => {
      const state = { ...initialState } as ToolStore;
      const result = fiSkillStoreSelectors.getServerByIdentifier('non-existent')(state);
      expect(result).toBeUndefined();
    });

    it('should return undefined when fiSkillServers is undefined', () => {
      const state = { ...initialState, fiSkillServers: undefined } as unknown as ToolStore;
      const result = fiSkillStoreSelectors.getServerByIdentifier('linear')(state);
      expect(result).toBeUndefined();
    });
  ...
  describe('isFiSkillServer', () => {
    it('should return true for existing server', () => {
      ...
      const state = { ...initialState, fiSkillServers: servers } as ToolStore;
      const result = fiSkillStoreSelectors.isFiSkillServer('linear')(state);
      expect(result).toBe(true);
    });

    it('should return false for non-existing server', () => {
      const state = { ...initialState } as ToolStore;
      const result = fiSkillStoreSelectors.isFiSkillServer('non-existent')(state);
      expect(result).toBe(false);
    });

    it('should return false when fiSkillServers is undefined', () => {
      const state = { ...initialState, fiSkillServers: undefined } as unknown as ToolStore;
      const result = fiSkillStoreSelectors.isFiSkillServer('linear')(state);
      expect(result).toBe(false);
    });
  ...
  describe('isServerLoading', () => {
    it('should return true when server is loading', () => {
      const state = {
        ...initialState,
        fiSkillLoadingIds: new Set(['linear']),
      } as ToolStore;
      const result = fiSkillStoreSelectors.isServerLoading('linear')(state);
      expect(result).toBe(true);
    });
    ...
    it('should return false when fiSkillLoadingIds is undefined', () => {
      const state = {
        ...initialState,
        fiSkillLoadingIds: undefined,
  ...
  describe('isToolExecuting', () => {
    it('should return true when tool is executing', () => {
      const state = {
        ...initialState,
        fiSkillExecutingToolIds: new Set(['linear:createIssue']),
      } as ToolStore;
      const result = fiSkillStoreSelectors.isToolExecuting('linear', 'createIssue')(state);
      expect(result).toBe(true);
    });
    ...
    it('should return false for different tool', () => {
      ...
      const result = fiSkillStoreSelectors.isToolExecuting('linear', 'listIssues')(state);
      expect(result).toBe(false);
    });

    it('should return false when fiSkillExecutingToolIds is undefined', () => {
      const state = {
        ...initialState,
        fiSkillExecutingToolIds: undefined,
      } as unknown as ToolStore;
      const result = fiSkillStoreSelectors.isToolExecuting('linear', 'createIssue')(state);
      expect(result).toBe(false);
    });
  ...
  describe('getAllTools', () => {
    it('should return all tools from connected servers', () => {
      ...
      const state = { ...initialState, fiSkillServers: servers } as ToolStore;
      const result = fiSkillStoreSelectors.getAllTools(state);
  ...
  describe('fiSkillAsLobeTools', () => {
    it('should convert connected servers with tools to FiTool format', () => {
      const servers: FiSkillServer[] = [
        {
          identifier: 'linear',
          name: 'Linear',
      ...
      const state = { ...initialState, fiSkillServers: servers } as ToolStore;
      const result = fiSkillStoreSelectors.fiSkillAsLobeTools(state);
  ...
  describe('metaList', () => {
    it('should return metadata for connected servers', () => {
      ...
      const state = { ...initialState, fiSkillServers: servers } as ToolStore;
      const result = fiSkillStoreSelectors.metaList(state);

      expect(result).toHaveLength(1);
      expect(result[0].identifier).toBe('linear');
      expect(result[0].meta.title).toBe('Linear');
      expect(result[0].meta.avatar).toBe('linear-icon');
      expect(result[0].meta.description).toBe('Fi Skill: Linear');
    });
