...
export abstract class BaseService implements IBaseService {
  ...
  protected async resolveOperationPermission(
     ...
     */
    if (hasGlobalAccess) {
      this.log('info', `Permission granted: current user has the highest ${permissionKey} permission`, logContext);
      return {
    ...
    if (!resourceBelongTo || resourceBelongTo !== this.userId) {
      this.log('warn', 'Permission denied: current user lacks ALL permission, or target resource does not belong to current user', logContext);
      return {
        isPermitted: false,
    ...
    if (resourceBelongTo === this.userId) {
      ...
      this.log('warn', 'Permission denied: target resource belongs to current user, but user lacks the required owner permission for this operation', logContext);
      return {
        isPermitted: false,
  ...
  protected async resolveBatchQueryPermission(
    ...
    if (allBelongToCurrentUser) {
      ...
      if (hasOwnerAccess) {
        this.log(
          'info',
          `Permission granted: batch operation, all resources belong to current user with ${permissionKey} owner permission`,
        );
