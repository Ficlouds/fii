...
/**
 * Locale candidate priority: full code (zh-CN) → base code (zh) → other codes sharing the same base (zh-HK) → default field.
 * Only matches keys that exist in i18n, to avoid overwriting defaults with undefined as a valid value.
 */
...
/**
 * Resolves billboard-level fields (currently only title, used for ? menu display).
 */
