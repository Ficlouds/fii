...
export function generateRootManPage(program: Command, version: string) {
  ...
  const lines = [
    ...
    `.TH LH 1 "" "${escapeRoff(`@ficlouds/cli ${version}`)}" "User Commands"`,
    '.SH NAME',
    `lh \\- ${escapeRoff(program.description() || 'Fi CLI')}`,
    '.SH SYNOPSIS',
    ...formatSynopsisLines(),
    '.SH DESCRIPTION',
    escapeRoff(
      `${program.name()} is the command-line interface for Fi. It provides authentication, device gateway connectivity, content generation, resource search, and management commands for agents, files, models, providers, plugins, knowledge bases, threads, topics, and related resources.`,
    ),
    '.PP',
