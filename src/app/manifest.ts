...
const manifest = async (): Promise<MetadataRoute.Manifest> => {
  ...
  if (process.env.NODE_ENV === 'development') {
    return {
      ...
      description: 'Fi Development',
      display: 'standalone',
      icons: [
        {
          sizes: '192x192',
          src: '/logos/fi-icon.svgicon-192x192.png',
          type: 'image/png',
        },
      ],
      name: 'Fi',
  ...
  const [{ BRANDING_LOGO_URL, BRANDING_NAME }, { kebabCase }, { manifestModule }] =
    await Promise.all([
      import('@ficlouds/business-const'),
    ...
    icons: [
      {
        purpose: 'any',
        sizes: '192x192',
        url: '/logos/fi-icon.svgicon-192x192.png',
      },
      {
        purpose: 'maskable',
        sizes: '192x192',
        url: '/logos/fi-icon.svgicon-192x192.maskable.png',
      },
      {
        purpose: 'any',
        sizes: '512x512',
        url: '/logos/fi-icon.svgicon-512x512.png',
      },
      {
        purpose: 'maskable',
        sizes: '512x512',
        url: '/logos/fi-icon.svgicon-512x512.maskable.png',
      },
    ],
