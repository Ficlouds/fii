...
export const createRuntimeExecutors = (
  ...
  call_llm: async (instruction, state) => {
    ...
    // Parent existence preflight (fail-fast on missing parent instead of silent swallow):
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        try {
          ...
          const response = await modelRuntime.chat(chatPayload, {
            callback: {
              ...
              onToolsCalling: async ({ toolsCalling: raw }) => {
                ...
                // replaying history. Invalid arguments are sanitized to prevent provider 400 errors.
          ...
          try {
            ...
            // builds and 400 strict providers like NVIDIA NIM. Invalid tool_call arguments are sanitized.
  ...
  call_tool: async (instruction, state) => {
    ...
    try {
      ...
      // either duplicate the tool_call_id or violate parent_id FK constraint.
      ...
      // running the agent on a broken conversation chain. Parent existence is checked before LLM call.
      ...
      toolsToExecute.map(async (chatToolPayload: ChatToolPayload) => {
        ...
        try {
          ...
          try {
            ...
            // the ConversationParentMissing throw is consumed (fail-fast instead of silent continue).
            const fatal = isParentMessageMissingError(error)
    ...
    if (skipCreateToolMessage) {
      ...
      for (const toolPayload of pendingToolsCalling) {
        ...
        try {
          ...
          // two entries for the same tool_call_id — skipCreateToolMessage prevents duplicate inserts.
  ...
  resolve_aborted_tools: async (instruction, state) => {
    ...
    for (const toolPayload of toolsCalling) {
      ...
      try {
        ...
        // error instead of the raw driver text (structured error propagation).
        const fatal = isParentMessageMissingError(error)
          ? createConversationParentMissingError(parentMessageId, error)
          : error instanceof Error
            ? error
