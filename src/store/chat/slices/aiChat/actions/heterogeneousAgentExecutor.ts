...
/**
 * Subscribe to Electron IPC broadcasts. As of Cloud CC V3 phase 0, the main
...
export const executeHeterogeneousAgent = async (
  ...
  try {
    ...
    /**
     * Process a single `AgentStreamEvent` from main. As of Cloud CC V3 phase 0,
    ...
    const handleStreamEvent = (event: AgentStreamEvent) => {
      ...
      if (event.type === 'stream_start' && event.data?.newStep) {
        ...
        // External-signal context (Phase 1: metadata.signal): set when the adapter
        ...
        //
        // Phase 1 lives in metadata; Phase 2 promotes to a dedicated signal column
        ...
        persistQueue = persistQueue.then(async () => {
          ...
          // in the run (rescues toolless middle steps via lastToolMsgIdEver), then
      ...
      //     warnings even after the DB has settled (subagent event leak fix).
    ...
    // Cloud CC V3 phase 0; pull it back through the existing `getSessionInfo`
