import type { BusinessEdgeConfigData } from '@ficlouds/business-config/server';
...
export interface BillboardItem {
   ...
   * Keys use Fi locale codes (e.g. `zh-CN`, `en-US`, `ja-JP`).
   */
...
export interface BillboardSet {
  /** ISO timestamp — end of the display time window; Fi stops showing after this time */
  endAt: string;
  /**
  ...
  /** Unique identifier */
  slug: string;
  /** ISO timestamp — start of the display time window; Fi does not show before this time */
  startAt: string;
...
export interface EdgeConfigData extends BusinessEdgeConfigData {
   ...
   * dev deployments point to the dev store, prod deployments point to the prod store, transparent to the Fi side.
   */
