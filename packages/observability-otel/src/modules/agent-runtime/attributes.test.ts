...
describe('agent-runtime attribute builders', () => {
  it('builds invoke_agent attributes and drops undefined fields', () => {
    ...
    expect(attrs).toEqual({
      ...
      'fi.agent.operation.id': 'op_xyz',
      'fi.agent.step.index': 0,
    });
  ...
  it('builds invoke_agent result attributes with completion reason', () => {
    ...
    expect(attrs).toEqual({
      ...
      'fi.agent.step.count': 5,
      'fi.agent.completion_reason': 'done',
    });
  });
  ...
  it('builds execute_tool attributes including tool type and call id', () => {
    ...
    expect(attrs).toMatchObject({
      ...
      'fi.tool.source': 'builtin',
      'fi.agent.operation.id': 'op_xyz',
      'fi.agent.step.index': 1,
    });
  });
  ...
  it('builds context_engineering attributes for snapshot-level metadata', () => {
    ...
    expect(attrs).toMatchObject({
      'fi.context.message_count': 12,
      'fi.context.knowledge_injected': true,
      'fi.context.knowledge_count': 3,
      'fi.context.history_compressed': false,
      'fi.context.memory_injected': true,
      'fi.context.system_role_length': 1024,
      'fi.context.tool_count': 7,
      'fi.context.has_images': true,
      'fi.agent.operation.id': 'op_xyz',
      'fi.agent.step.index': 0,
    });
    expect(attrs).not.toHaveProperty('fi.context.token_usage');
    expect(attrs).not.toHaveProperty('fi.context.window_ratio');
  });
