...
export interface GlobalBillboardItem {
  ...
  description: string;
  /**
   * Per-locale copy overrides. Falls back to the default fields when a locale or a field within the locale is missing.
   */
...
export interface GlobalBillboard {
  endAt: string;
  /**
   * Per-locale overrides for billboard-level fields (currently only title). Falls back to the default title when missing.
   */
