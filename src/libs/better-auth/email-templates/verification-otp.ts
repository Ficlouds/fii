/**
 * Email OTP verification template
 * Microsoft-style — clean, minimal, no boxes
 */
export const getVerificationOTPEmailTemplate = (params: {
  expiresInSeconds: number;
  otp: string;
  userName?: string | null;
}) => {
  const { otp, userName, expiresInSeconds } = params;
  const expiresInMinutes = Math.round(expiresInSeconds / 60);
  const expirationText = expiresInMinutes >= 1
    ? `${expiresInMinutes} minute${expiresInMinutes > 1 ? 's' : ''}`
    : `${expiresInSeconds} seconds`;
  const year = new Date().getFullYear();
  const greeting = userName ? `Hi ${userName},` : 'Hi,';

  return {
    subject: 'Your Fi verification code',
    text: `${greeting}\n\nYour Fi verification code is: ${otp}\n\nThis code expires in ${expirationText}.\n\nDo not share this code with anyone.\n\nThanks,\nThe Fi team`,
    html: `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Your Fi verification code</title>
</head>
<body style="margin:0;padding:0;background-color:#f9f9f9;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Helvetica Neue',Arial,sans-serif;color:#111111;">
  <div style="max-width:560px;margin:0 auto;padding:0;">

    <!-- Header strip -->
    <div style="background-color:#111111;padding:24px 40px;">
      <img src="data:image/png;base64," alt="Fi" style="height:28px;width:auto;display:block;">
    </div>

    <!-- Body -->
    <div style="background-color:#ffffff;padding:40px 40px 32px;">

      <h1 style="font-size:22px;font-weight:700;color:#111111;margin:0 0 20px;letter-spacing:-0.3px;line-height:1.3;">
        Fi account verification
      </h1>

      <p style="font-size:15px;color:#333333;margin:0 0 8px;line-height:1.7;">
        ${greeting}
      </p>

      <p style="font-size:15px;color:#333333;margin:0 0 28px;line-height:1.7;">
        Please use this code to verify your Fi account.
      </p>

      <!-- Code section -->
      <p style="font-size:14px;color:#555555;margin:0 0 10px;font-weight:500;">
        Here is your code:
      </p>
      <p style="font-size:32px;font-weight:700;color:#111111;margin:0 0 28px;letter-spacing:8px;font-family:'Courier New',Courier,monospace;">
        ${otp}
      </p>

      <p style="font-size:14px;color:#555555;margin:0 0 8px;line-height:1.7;">
        This code expires in <strong>${expirationText}</strong>. Do not share this code with anyone.
      </p>

      <p style="font-size:14px;color:#888888;margin:0 0 36px;line-height:1.7;">
        If you did not request this code, you can safely ignore this email. Your account has not been affected.
      </p>

      <p style="font-size:14px;color:#333333;margin:0 0 2px;">Thanks,</p>
      <p style="font-size:14px;color:#333333;margin:0 0 36px;">The Fi team</p>

    </div>

    <!-- Footer -->
    <div style="background-color:#f9f9f9;padding:24px 40px;border-top:1px solid #e5e5e5;">
      <p style="font-size:12px;color:#888888;margin:0 0 4px;line-height:1.6;">
        Fi by Ficlouds &nbsp;&middot;&nbsp;
        <a href="https://ficlouds.com/privacy" style="color:#888888;text-decoration:none;">Privacy Statement</a>
      </p>
      <p style="font-size:12px;color:#aaaaaa;margin:0;">
        &copy; ${year} Ficlouds. All rights reserved.
      </p>
    </div>

  </div>
</body>
</html>`,
  };
};
