 ...
 * tenant_id)` so a single Fi deployment can serve many workspaces of the
 * same Slack App without collisions.
 *
 * Distinct from `agent_bot_providers` (per-agent-per-user bot bindings): this
 * row represents the Fi-owned, Marketplace-distributed app installed
...
export const messengerInstallations = pgTable(
  'messenger_installations',
  {
    ...
    /** Fi user who clicked "Connect Slack" — bound at OAuth callback. */
