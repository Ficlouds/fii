...
// Default state mocks for single-human thread relaxation. Tests that
...
// keep their pre-watch-keyword behaviour. Individual tests can replace this via
...
vi.mock('../platforms', () => ({
  ...
  // Mirror the production helpers just well enough that (watch-keyword wake)
...
describe('BotMessageRouter', () => {
  beforeEach(() => {
    ...
    // Participant tracking — restore defaults wiped by (single-human thread relaxation)
  ...
  describe('onSubscribedMessage policy', () => {
    // Single-human thread relaxation: a non-mention
    ...
    it('should skip non-mention messages in a multi-human group thread', async () => {
      // Post-fix: the gate keys off thread.isDM || mention || (single-human relaxation)
    ...
    it('should not change behaviour when watchKeywords is empty/missing', async () => {
      // Sanity: existing call sites must keep their pre-watch-keyword semantics.
