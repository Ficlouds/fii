 ...
 * instrumentation, alongside Fi-specific (`fi.*`) extensions.
 *
...
// ---- fi.* (Fi-specific extensions) ----

/** Internal operation id assigned by the Agent Runtime. */
export const ATTR_FI_AGENT_OPERATION_ID = 'fi.agent.operation.id' as const;

/** Total step count for the agent invocation. */
export const ATTR_FI_AGENT_STEP_COUNT = 'fi.agent.step.count' as const;

/** Current step index within the agent invocation. */
export const ATTR_FI_AGENT_STEP_INDEX = 'fi.agent.step.index' as const;

/** Completion reason: `done` / `error` / `max_steps` / `cost_limit` / `interrupted` / `waiting_for_human`. */
export const ATTR_FI_AGENT_COMPLETION_REASON = 'fi.agent.completion_reason' as const;

/** Tool execution success flag (gen_ai spec uses error.type for failures only). */
export const ATTR_FI_TOOL_SUCCESS = 'fi.tool.success' as const;

/** Attempts taken to execute a tool (1 for first-try success). */
export const ATTR_FI_TOOL_ATTEMPTS = 'fi.tool.attempts' as const;

/** Internal Fi tool source (`builtin` / `client` / `mcp` / `klavis` / `fiSkill`). */
export const ATTR_FI_TOOL_SOURCE = 'fi.tool.source' as const;

/** Context engineering metadata. */
export const ATTR_FI_CONTEXT_MESSAGE_COUNT = 'fi.context.message_count' as const;
export const ATTR_FI_CONTEXT_TOKEN_USAGE = 'fi.context.token_usage' as const;
export const ATTR_FI_CONTEXT_WINDOW_RATIO = 'fi.context.window_ratio' as const;
export const ATTR_FI_CONTEXT_KNOWLEDGE_INJECTED =
  'fi.context.knowledge_injected' as const;
export const ATTR_FI_CONTEXT_KNOWLEDGE_COUNT = 'fi.context.knowledge_count' as const;
export const ATTR_FI_CONTEXT_HISTORY_COMPRESSED =
  'fi.context.history_compressed' as const;
export const ATTR_FI_CONTEXT_MEMORY_INJECTED = 'fi.context.memory_injected' as const;
export const ATTR_FI_CONTEXT_SYSTEM_ROLE_LENGTH =
  'fi.context.system_role_length' as const;
export const ATTR_FI_CONTEXT_TOOL_COUNT = 'fi.context.tool_count' as const;
export const ATTR_FI_CONTEXT_HAS_IMAGES = 'fi.context.has_images' as const;
