...
export const useSignIn = () => {
  ...
  const [lastAuthProvider] = useState(() => {
    try { return localStorage.getItem(LAST_AUTH_PROVIDER_KEY); } catch { return null; }
  });
  ...
  const resolveEmailFromIdentifier = async (identifier: string): Promise<ResolvedEmailResult | null> => {
    const trimmedIdentifier = identifier.trim();
    if (!trimmedIdentifier) return null;
    const isEmailIdentifier = EMAIL_REGEX.test(trimmedIdentifier);
    if (isEmailIdentifier) return { email: trimmedIdentifier.toLowerCase(), identifierType: 'email' };
  ...
  const handleSendOtp = async (targetEmail?: string) => {
    ...
    try {
      await emailOtp.sendVerificationOtp({ email: emailToUse, type: 'sign-in' });
    } catch (error) {
  ...
  const handleCheckUser = async (values: Pick<SignInFormValues, 'email'>) => {
    ...
    try {
      ...
      if (!data.exists) {
        ...
        router.push(`/signup?email=${encodeURIComponent(targetEmail)}&callbackUrl=${encodeURIComponent(callbackUrl)}`);
        return;
      }
  ...
  const handleSocialSignIn = async (provider: string) => {
    ...
    await trackLoginOrSignupClicked({ provider: normalizedProvider, spm: 'signin.social.click' });
    try {
      if (ENABLE_BUSINESS_FEATURES && !(await preSocialSigninCheck())) { setSocialLoading(null); return; }
      try { localStorage.setItem(LAST_AUTH_PROVIDER_KEY, provider); } catch {}
      const callbackUrl = searchParams.get('callbackUrl') || '/';
      const additionalData = await getAdditionalData();
      const result = isBuiltinProvider(normalizedProvider)
        ? await signIn.social({ additionalData, callbackURL: callbackUrl, provider: normalizedProvider })
        : await signIn.oauth2({ additionalData, callbackURL: callbackUrl, providerId: normalizedProvider });
  ...
  const handleBackToEmail = () => { setStep('email'); setEmail(''); setIsSocialOnly(false); };
  ...
  const sortedProviders = lastAuthProvider
    ? [...resolvedProviders].sort((a, b) => { if (a === lastAuthProvider) return -1; if (b === lastAuthProvider) return 1; return 0; })
    : resolvedProviders;
