...
export class DiscoverService {
  ...
  async registerClient({ userAgent }: { userAgent?: string }) {
    ...
    const { client_id, client_secret } = await this.market.registerClient({
      clientName: `Fi ${isDesktop ? 'Desktop' : 'Web'}`,
    ...
    try {
      ...
      const assistant = {
        ...
        homepage:
          ...
          `https://ficlouds.com/discover/assistant/${(data as any).identifier}`,
    ...
    try {
      ...
      const transformedItems: DiscoverAssistantItem[] = (data.items || []).map((item: any) => {
        ...
        return {
          ...
          homepage: item.homepage || `https://ficlouds.com/discover/assistant/${item.identifier}`,
          identifier: item.identifier,
    ...
    try {
      ...
      const items: DiscoverAssistantItem[] = (data.items || []).map((item: any) => {
        ...
        return {
          ...
          homepage: `https://ficlouds.com/discover/assistant/${item.identifier}`,
          identifier: item.identifier,
          installCount: item.installCount,
  ...
  getPluginDetail = async (params: {
    ...
    const { builtinTools } = await import('@ficlouds/builtin-tools');
    ...
    if (withReadme) {
      ...
      try {
        ...
        const readmeUrl = urlJoin(
          'https://raw.githubusercontent.com/ficlouds/fi/refs/heads/main/docs/usage/providers',
  ...
  getUserInfo = async (params: {
    ...
    try {
      ...
      const transformedAgents: DiscoverAssistantItem[] = (agents || []).map((agent: any) => ({
        ...
        homepage: `https://ficlouds.com/discover/assistant/${agent.identifier}`,
      ...
      const transformedPlugins: DiscoverPluginItem[] = (plugins || []).map((plugin: any) => ({
        ...
        homepage: `https://ficlouds.com/discover/plugin/${plugin.identifier}`,
