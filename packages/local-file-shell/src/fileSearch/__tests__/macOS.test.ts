...
describe('buildFilenameKeywordExpression', () => {
  ...
  it('splits whitespace-separated keywords into AND-ed substring terms', () => {
    // Critical fix: a free-form keyword string from the LLM (e.g. "Fi
    ...
    expect(buildFilenameKeywordExpression('Fi Financial Statement')).toBe(
      '(kMDItemFSName == "*Fi*"cd && kMDItemFSName == "*Financial*"cd && kMDItemFSName == "*Statement*"cd)',
    );
  });
