...
const Item = memo<ItemProps>(
  ({ description, icon, identifier, label, onOpenDetail, serverName, type }) => {
    ...
    const i18nPrefix = type === 'klavis' ? 'tools.klavis.servers' : 'tools.fiSkill.providers';
    // @ts-ignore
    ...
    const confirmDisconnect = () => {
      confirmModal({
        ...
        content: t('tools.fiSkill.disconnectConfirm.desc', { name: label }),
        okButtonProps: { danger: true },
        okText: t('tools.fiSkill.disconnect'),
        onOk: handleDisconnect,
        title: t('tools.fiSkill.disconnectConfirm.title', { name: label }),
      });
    };
    ...
    const renderAction = () => {
            ...
            items={[
              {
                ...
                key: 'disconnect',
                label: t('tools.fiSkill.disconnect'),
      ...
      return (
        <ActionIcon icon={Plus} title={t('tools.fiSkill.connect')} onClick={handleConnect} />
      );
    };
